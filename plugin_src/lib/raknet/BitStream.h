...
public:
	...
	bool ReadCharPtr(char* output, const int numberOfBytes);
	...
	/// \param[in] numberOfBytes The number of byte to read
	/// \return true on success false if there is some missing bytes. 
	bool ReadArray(char* output, const int numberOfBytes);
...
template <class T, class... Args>
inline bool BitStream::Read(T& var, Args&... arg)
{
	bool result = ReadBits((unsigned char*)&var, sizeof(T) * 8, true);
	Read(arg...);
	return result;
}
